"""Helius data source for Solana blockchain data."""
...
def map_helius_token_to_snapshot(
    data: dict[str, Any], source: str = "helius"
) -> TokenSnapshot:
    """Map Helius API response to TokenSnapshot.
    ...
    price_usd = market_data.get("price", 0.0)
    volume_24h = market_data.get("volume24h", 0.0)
    liquidity = market_data.get("liquidity", 0.0)

    # Extract pool info if available
    pool_data = market_data.get("pool", {})
    pool = None
    if pool_data:
        pool = PoolId(
            program=pool_data.get("program", ""), address=pool_data.get("address", "")
        )
    ...
    return TokenSnapshot(
        ...
        pct_change_5m=market_data.get("priceChange5m", 0.0),
        source=source,
        ts=ts,
    )


class HeliusDataSource(MarketDataSource):
    """Helius API data source for Solana blockchain data."""
    ...
    async def _make_request(
        ...
        async for attempt in self.retry_config:
            try:
                ...
                logger.warning(
                    "Network error in Helius request",
                    endpoint=endpoint,
                    error=str(e),
                    attempt=attempt.retry_state.attempt_number,
                )
    ...
    async def poll(self) -> list[TokenSnapshot]:
        ...
        try:
            ...
            endpoint = "v0/tokens/trending"
            params = {"limit": 50}

            response_data = await self._make_request(endpoint, params)

            snapshots = []
            for token_data in response_data.get("tokens", []):
                try:
                    snapshot = map_helius_token_to_snapshot(token_data)
                    snapshots.append(snapshot)
                except Exception as e:
                    logger.warning(
                        "Failed to map token data",
                        token_mint=token_data.get("token", {}).get("mint"),
                        error=str(e),
                    )
                    continue

            logger.info("Polled Helius data", count=len(snapshots))
            return snapshots

        except Exception as e:
            logger.error("Failed to poll Helius data", error=str(e))
            return []
    ...
    async def lookup(self, token: TokenId) -> TokenSnapshot | None:
        ...
        try:
            ...
            endpoint = f"v0/tokens/{token.mint}"

            response_data = await self._make_request(endpoint)

            snapshot = map_helius_token_to_snapshot(response_data)
